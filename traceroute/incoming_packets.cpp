...
bool unicalping(std::string incomingips[3], std::string ip, int numberofindividualpings)
{
...
int calculateDuration(const std::chrono::time_point<Clock> &end, const std::chrono::time_point<Clock> &start)
{
    auto diff = std::chrono::duration_cast<Duration>(end - start);
    return diff.count();
}
int incoming_packet(int sockfd, int ttl, struct sockaddr_in *last_router, long total_time, std::chrono::time_point<Clock> tab[3])
{
    ...
    while (calculateDuration(accttime, timewaitstart) <= 1000 and recivedpings < 3)
    {
        accttime = Clock::now();
        ...
        if (poll(descriptors, 1, POLLTIMER))
        {
            ...
            ssize_t packet_len = recvfrom(sockfd, buffer, IP_MAXPACKET, MSG_DONTWAIT, (struct sockaddr *)&router, &router_len);
            if (packet_len < 0)
            {
                std::cout << "Error receiving packet\n";
                return EXIT_FAILURE;
            }
            if (inet_ntop(AF_INET, &(router.sin_addr), iptoprint, sizeof(iptoprint)) == NULL)
            {
